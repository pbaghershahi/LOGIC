...
import ipdb
from utils import custom_k_hop_subgraph


from TAGE.objectives import JSE_loss, NCE_loss
...
class TAGExplainer(nn.Module):
	...
	def get_subgraph(self, node_idx: int, data: Data):
		...
		# subset, edge_index, mapping, edge_mask = utils.k_hop_subgraph(6, 2, edge_index, relabel_nodes=True)
	...
	def train_explainer_graph(self, loader, lr=0.001, epochs=10):
		...
		for epoch in range(epochs):
			...
			for data in pbar:
				...
				pruned_embed, mask, log = self.explain(data, embed=node_embed, condition=cond, tmp=tmp, training=True)
			...
			for dt_idx, data in enumerate(loader):
				...
				for node_batch in pbar:
					...
					for node_idx in node_batch:
						...
						with torch.no_grad():
							_, subg_embeds = self.model(batch = subgraph)
	...
	def train_explainer_node(self, train_loader, batch_size=128, lr=0.001, epochs=10):
		""" training the explanation network by gradient descent(GD) using Adam optimizer """
		optimizer = Adam(self.explainer.parameters(), lr=lr)
			# train the mask generator
		...
		for epoch in range(epochs):
			...
			for dt_idx, batch_idxs in enumerate(train_loader):
				loss = 0.0
				optimizer.zero_grad()
				tmp = float(self.t0 * np.power(self.t1 / self.t0, epoch / epochs))
				...
				with torch.no_grad():
					_, all_embeds = self.model(batch = train_ds)
				# self.model.to(self.device)
				batch_idxs = batch_idxs.to(self.device)

				# node_batches = torch.utils.data.DataLoader(
				# 	torch.arange(batch_size).tolist(),
				...
				# for node_batch in pbar:
				# 	cond = self.__rand_cond__(1)
				# 	embeds = all_embeds[node_batch]
				# 	pruned_embeds = []
				# 	masks = []
				# 	for node_idx in node_batch:
				# 		# subgraph, subset = self.get_subgraph(node_idx=node_idx, data=train_ds)

				# 		subset, edge_index, mapping, edge_mask = utils.k_hop_subgraph(node_idx, 2, train_ds.edge_index, relabel_nodes=True)
				...
				for node_idx in batch_idxs:
					...
					pruned_embeds.append(pruned_embed.to(self.device)[mapping:mapping+1])
					masks.append(mask)
	...
	def forward(self, data: Data, mlp_explainer: nn.Module, **kwargs):
		...
		if node_idx is not None:
			_, node_embed = self.model(batch = data)
		...
		if self.explain_graph:
			...
			_, masked_embed = self.model(batch = masked_data)
			...
			# 	flag2 = (edge_index == edge_index1).all()
			# 	flag3 = (mapping == mapping1).all()
			# 	flag4 = (edge_mask == edge_mask1).all()
			# 	# print(flag1, flag2, flag3, flag4)
			# except:
			# 	ipdb.set_trace()					
			
			mapping = mapping.item()
			subgraph = Data(x=data.x[subset], edge_index=edge_index, device=self.device)

			_, edge_mask, log = self.explain(
				subgraph, 
				node_embed[subset].to(self.device), 
				condition=grads, 
				tmp=1.0, 
				training=False, 
				node_idx=mapping
			...
			# masked_data = mask_fn(subgraph, node_mask)
			# NOTE: The original code doesn't append the ego node to the maked_subset as in the following lines. This is to align all experimental settings. 
			if node_mask is not None:
				if node_mask.any():
					masked_subset = list(set(node_mask.nonzero()[0].tolist() + [mapping]))
				else:
					masked_subset = [mapping]

			masked_data = Data(
				x = data.x[subset], 
				edge_index = utils.subgraph(masked_subset, edge_index, relabel_nodes=True)[0]
			)
			# _, masked_embed = self.model(batch = masked_data)
			# masked_prob = mlp_explainer(masked_embed, mode='pred')[mapping.item(), target_class.long()]
			# masked_prob, _ = self.model(batch = masked_data)[mapping.item(), target_class.long()]
			# sparsity_score = sum(node_mask) / subgraph.num_nodes

		# ipdb.set_trace()
		# return variables
		# edge_mask = edge_mask.detach().to(self.device)
		# related_preds = [{
		# 	'maskout': masked_prob.item(),
		# 	'origin': probs[target_class].item(),
		# 	'sparsity': sparsity_score}]
		return masked_data, subset, masked_subset, mapping
