...
def train_MLP(embed_model, mlp_model, train_loader, val_loader, save_to=None):
    ...
    for _ in range(epochs):
        ...
        mlp_model.train()
        # ipdb.set_trace()
        for step, batch_idxs in enumerate(tqdm(train_loader, desc="Iteration")):
            _, embeds = embed_model(batch = train_ds)
            pred = mlp_model(embeds.detach())[batch_idxs]
            # y = train_ds.y[batch_idxs].to(device).view(pred.shape).to(torch.float64)
            y = train_ds.y[batch_idxs].to(device)
        ...
        print("====Evaluation")
        for step, batch_idxs in enumerate(tqdm(val_loader, desc="Iteration")):

            with torch.no_grad():
                _, embeds = embed_model(batch = val_ds)
                pred = mlp_model(embeds.detach())[batch_idxs]

            y_true.append(val_ds.y[batch_idxs].to(device))
            y_scores.append(pred.argmax(dim=1))
        ...
        print(f"Acc on validation set: {(y_scores == y_true).mean():.3f}")
